...
log += "<p>CDM device: {} (system ID: {}, security level: {})".format(device.system_id, device.security_level)
cdm = Cdm.from_device(device)
session_id = cdm.open()
log += "<p>CDM session: {}".format(session_id.hex())
challenge = cdm.get_license_challenge(session_id, pypssh)
log += "<p>Created License Request Message (Challenge): {}".format(challenge.hex())
have_license = False
try:
